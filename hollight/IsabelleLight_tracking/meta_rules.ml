...
let (MDISCH_TAC: tactic) =
  fun (asl,w) ->
    try let ant,c = dest_mimp w in
       ...
       simple_rose( null_meta,[("",th1)::asl,c],
...
let REWRITE_META_RULE:thm list->meta_rule->meta_rule =
  fun thl (c,glist,j) ->
    ...
    and rewrg = (hd o snd3 o (fun a,b,c,_ -> a,b,c) o (REWRITE_ASM_TAC thl THEN REWRITE_TAC thl)) in
    rewr c,
    map (rewrg) glist,
...
let (rulem_tac: (term*term) list->meta_rule->tactic) =
  fun instlist r ((asl,w) as g) ->
    ...
    simple_rose((mvs,null_inst),new_goals,fun i l ->
