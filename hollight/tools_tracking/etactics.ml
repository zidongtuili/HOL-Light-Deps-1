...
let (EVALID:'a etactic->'a etactic) =
  ...
  fun tac state (asl,w) ->
    let (((mvs,i),gls,just,_),_ as res) = tac state (asl,w) in
...
let ETHEN,ETHENL =
  ...
  let rec seqapply ext l1 l2 = match (l1,l2) with
    ...
    | ((tac:'a etactic)::tacs),((goal:goal)::goals) ->
        let (((mvs1,insts1),gls1,just1,_),ext1) = tac ext goal in
  ...
  let tacsequence (((mvs1,insts1),gls1,just1),ext1) tacl =
    ...
    (simple_rose((union mvs1 mvs2,compose_insts insts1 insts2),gls2,just),ext2) in
  let (then_: 'a etactic -> 'a etactic -> 'a etactic) =
    fun tac1 tac2 s g ->
    let (a,gls,b,_),c  = tac1 s g in
    tacsequence ((a,gls,b),c) (replicate tac2 (length gls))
  and (thenl_: 'a etactic -> 'a etactic list -> 'a etactic) =
    fun tac1 tac2l s g ->
    let (a,gls,b,_),c  = tac1 s g in
    if gls = [] then tacsequence ((a,gls,b),c ) []
    else tacsequence ((a,gls,b),c ) tac2l in
  then_,thenl_
;;
...
let (ALL_ETAC:'a etactic) =
  fun s g -> simple_rose (null_meta,[g],fun _ [th] -> th),s;;

let (ALL_ETAC':('a -> 'a) -> 'a etactic) =
  fun f s g -> simple_rose (null_meta,[g],fun _ [th] -> th),f s;;
...
let (CHANGED_ETAC: 'a etactic -> 'a etactic) =
  fun tac s g ->
    let ((meta,gl,_,_),_ as gstate) = tac s g in 
...
let (EVALID:'a etactic->'a etactic) =
  ...
  let (((mvs,i),gls,just,_),_ as res) = tac s (asl,w) in
  let ths = map fake_thm gls in
  let asl',w' = dest_thm(just null_inst ths) in
  let asl'',w'' = inst_goal i (asl,w) in
  let maxasms =
...
let (eby:'a etactic->goalstate * 'a -> goalstate * 'a) =
  fun tac (((mvs,inst),gls,just,_),state) ->
    if gls = [] then failwith "No goal set" else
    let g = hd gls
    and ogls = tl gls in
    let ((newmvs,newinst),subgls,subjust,_),state' = tac state g in
