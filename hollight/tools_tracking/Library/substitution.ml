...
(*
(* ========================================================================= *)
(* Small library that generates substitutions with fresh variables.          *)
(* ========================================================================= *)
...
(*
let FRESH = define
  `FRESH ch avoids (v:A) = if MEM v avoids
    then FEMPTY |+ (v,ch v avoids)
    else (FEMPTY:(A,A)fmap)`;;
*)



let FRESHL = new_recursive_definition list_RECURSION
  `(FRESHL ch avoids [] = FEMPTY) /\
  (FRESHL ch avoids (CONS (h:A) t) =
      if MEM h avoids
      then
	let h' = ch h avoids in FRESHL ch (CONS h' avoids) t |+ (h,h')
      else
	FRESHL ch avoids t)`;;
...
let FRESHL_EMPTY = prove (
  `!ch l. FRESHL ch [] l = FEMPTY`,
  GEN_TAC THEN LIST_INDUCT_TAC THEN ASM SIMP_TAC[FRESHL;MEM]);;

let FRESHL_LDIFF_VAIN = prove (
  `!ch l v. FRESHL ch (v LDIFF l) l = FEMPTY`,
  GEN_TAC THEN LIST_INDUCT_TAC THEN GEN_TAC THEN ASM SIMP_TAC[FRESHL;NOT_MEM_LDIFF_CONS;LDIFF_CONS]);;
...
let CHOICE_HYP_NEQ = prove (
  `!ch x n l. (!S n. ~MEM (ch n S) S) ==> MEM x l ==> ~(x = ch n l)`,
  GEN_TAC THEN GEN_TAC THEN GEN_TAC THEN LIST_INDUCT_TAC THEN ASM MESON_TAC[MEM]);;

g `!l x v. (!S (n:A). ~(MEM (ch n S) S)) ==> ~(MEM x (MAP (SUB (FRESHL ch v l)) l) /\ (MEM x v))`;;
e (REWRITE_TAC[TAUT `!p q. ~(p /\ q) <=> q ==> ~p`] 
     THEN LIST_INDUCT_TAC THEN simp[FRESHL;MEM;MAP] THEN REPEAT GEN_TAC);;
e (COND_CASES_TAC);;

e (simp[SUBST]);;
e (REPEAT LET_TAC);;
e (REPEAT (FIRST_X_ASSUM (SUBST1_TAC o GSYM)));;
e (simp[FLOOKUP_FUPDATE;distinctness "option";THE_DEF]);;
e (REPEAT DISCH_TAC);;
e (FIRST_X_ASSUM (DISJ_CASES_TAC));;

e (meson[]);;


e (CONTEXT_THEN subgoal_tac `~(x=ch h v)`);;
e (simp[CHOICE_HYP_NEQ]);;
e (MATCH_MP_THEN ASSUME_TAC MEM_MAP_SUB_FUPDATE);;
e (FIRST_ASSUM (MATCH_MP_THEN ASSUME_TAC));;
e (simp[FRESHL]);;
e (meson[]);;
e (simp[MEM_MAP_SUB_FUPDATE]);;
e (rule ((MIMP_RULE o SPEC_ALL) CHOICE_HYP_NEQ));;
*)
