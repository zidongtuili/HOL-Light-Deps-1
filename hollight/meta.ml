...
(* let thm_setup thm_ids = *)
(*   let hook store_id i thm = *)
(*     let ident,vd = List.nth !id_vd_store i in *)
...
(*       | None -> ident_map in *)
(*     let dep_src_thms = fold_ident_str f Identmap.empty tree *)
(*                        |> Identmap.to_list *)
(*                        |> map snd in *)
(*     register_toplevel_thm ident vd dep_src_thms thm in *)
(*   auto_identify (List.map (fun (thm,store_id) -> thm, hook store_id) thm_ids);; *)
