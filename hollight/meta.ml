...
let mk_src_fns of_src =
  let (from_ident_map : 'a Meta.src Identmap.t ref) = ref Identmap.empty in
  ...
  register_ident, from_ident, srcs;;

(* Registration of thm identifiers. *)
let register_thm_ident, find_thm_src, thm_srcs =
  let mk_src src thms = { src with Meta.src_obj = thms } in
  let reg, find_from_ident, thm_srcs = mk_src_fns mk_src in
  (fun ident vd thms ->
   let loc = vd.Types.val_loc in
   let src = reg ident loc thms in
   src),
  find_from_ident,
  thm_srcs;;
...
let with_tracking_nodup thm =
  ...
  | _ ->
     ...
     | [] -> let id,thm = with_tracking thm in id,thm
     | [idthm] -> idthm
     | _ -> failwith "Theorem has two duplicates in its dependency graph."

let register_thm, get_thms =
  let (thms : thm list ref) = ref [] in
  (fun thm -> thms := thm :: !thms),
  fun () -> rev !thms;;
