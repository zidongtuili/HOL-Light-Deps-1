      ...
      Typedtreeiter.Makeiterator(
          struct
            ...
            let enter_expression exp =
              (* URGENT TODO: Paths need to be looked up in the corresponding
                 environment. *)
              match exp.Typedtree.exp_desc with
              | Typedtree.Texp_ident (p,_,_) -> f (Path.head p)
              | _ -> ()
          end) in
    Ident_iterator.iter_structure,
    Ident_iterator.iter_expression in
  (fun env f b t -> mk_fold (fst o folds env) f b t),
  (fun env f b t -> mk_fold (snd o folds env) f b t);;
...
let hook store_id thm_id thm =
  ...
  let f ident_map ident =
    ...
    | None -> ident_map in
  let env,trees = !rhs_tree in
  let dep_src_thms = List.fold_left (fold_ident_expr env f) Identmap.empty trees
                     |> Identmap.to_list
                     |> map snd in
  let src = register_thm_ident qualifiers ident vd ([],thm) in
  let meta = meta_of_thm thm_id thm src Meta.Toplevel dep_src_thms [] in
  register_thm_meta thm meta

let with_tracking_nodup store_id thm =
